...
export default function InterviewCommunication() {
  ...
  const getTableRowStyle = (isHeader = false) => ({
    display: 'grid',
    gridTemplateColumns: '0.8fr 1.2fr 1.2fr 0.8fr',
    background: isHeader ? colors.bg3 : colors.bg,
    padding: '8px 12px',
    gap: '8px',
    borderTop: isHeader ? 'none' : `1px solid ${colors.border}`,
  });
  ...
  return (
    <div style={{ padding: '4px 0', fontFamily: "'Space Mono', monospace" }}>
      <p style={{
        ...
        Your technical knowledge is solid. Your trainer's feedback? The missing piece isn't what you say—it's <strong style={{ color: colors.white }}>how</strong> you say it. 
        Eye contact, enthusiasm, and concise answers separate good candidates from hired ones. Here's the framework.
      </p>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <button 
          style={getTabButtonStyle(tab === 'before-after')}
          onClick={() => setTab('before-after')}
        >
          Before & After
        </button>
        <button 
          style={getTabButtonStyle(tab === 'practice')}
          onClick={() => setTab('practice')}
        >
          Practice Checklist
        </button>
        <button 
          style={getTabButtonStyle(tab === 'examples')}
          onClick={() => setTab('examples')}
        >
          Answer Examples
        </button>
      ...
      {/* Before & After */}
      {tab === 'before-after' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', borderRadius: '8px', overflow: 'hidden', border: `1px solid ${colors.border}` }}>
          <div style={getTableRowStyle(true)}>
            <span style={getCellStyle(colors.muted)}>Aspect</span>
            <span style={{ ...getCellStyle(colors.error), fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Before (You)</span>
            <span style={{ ...getCellStyle(colors.success), fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.08em' }}>After (Hired)</span>
            <span style={{ ...getCellStyle(colors.warning), fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Priority</span>
          </div>
          {beforeAfter.map(({ aspect, before, after, impact }) => (
            <div key={aspect} style={getTableRowStyle()}>
              <span style={getCellStyle(colors.text)}>{aspect}</span>
              <span style={getCellStyle(colors.error)}>{before}</span>
              <span style={getCellStyle(colors.success)}>{after}</span>
              <span style={getCellStyle(colors.warning)}>{impact}</span>
            </div>
          ))}
        </div>
      )}
          ...
          {practiceCheckpoints.map(({ emoji, title, desc }, i) => (
            <div 
              ...
              style={{
                ...
                gap: '14px',
                padding: '14px 16px',
                background: i % 2 === 0 ? colors.bg3 : colors.bg,
            ...
            <div key={question} style={{
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  ❌ Without Enthusiasm (Lost Candidate)
                </div>
                <div style={{
                  fontSize: '11px',
                  color: colors.muted,
                ...
                <div style={{
                  ...
                  → Flat delivery, rambling, unfocused
                </div>
              </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  ✓ With Enthusiasm (Hired Candidate)
                </div>
                <div style={{
                  fontSize: '11px',
                  color: colors.muted,
                ...
                <div style={{
                  ...
                  → Specific, energetic, shows impact & growth
                </div>
              </div>
            </div>
          ))}
